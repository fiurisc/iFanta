...
Namespace SystemFunction
    ...
    Public Class Mail
        ...
        'Public Shared Function SendRosa(ToAddress As String, Display As String, Subject As String, Body As String, AttachFileList As List(Of String)) As Boolean

        '    Dim att As New List(Of String)
        '    att.Add(My.Application.Info.DirectoryPath & "\error.log")
        '    att.Add(My.Application.Info.DirectoryPath & "\mt.tmp")
        '    SystemFunction.General.SendMail("fernando.iurisci@gmail.com", "", "Formazione 12 giornata", "rerere", att)

        'End Function

        Public Shared Function SendFormazione(ToAddress As String, CcAddress As String, Giornata As Integer, IdTeam As Integer, Torneo As String) As Boolean

            Dim formazione As New LegaObject.Formazione(Giornata, IdTeam)
            formazione.Load(False)

            Return SendFormazione(ToAddress, CcAddress, formazione, Torneo)

        End Function

        Public Shared Function SendFormazione(ToAddress As String, CcAddress As String, Formazione As LegaObject.Formazione, Torneo As String) As Boolean

            Dim body As New System.Text.StringBuilder
            body.AppendLine("Salve,")
            body.AppendLine("In allegato trovi la mia formazione per la " & Formazione.Giornata & "° giornata.")
            body.AppendLine()
            body.AppendLine("Cordiali saluti")
            body.AppendLine(Formazione.Allenatore)

            Return SendFormazione(ToAddress, CcAddress, Formazione, body.ToString, Torneo)

        End Function

        Public Shared Function SendFormazione(ToAddress As String, CcAddress As String, Formazione As LegaObject.Formazione, Body As String, Torneo As String) As Boolean

            Dim fname As String = ImpExp.ImpExpFormazioni.ExportHtml(Formazione.Giornata, Formazione, False, True)
            ...
            If fname <> "" Then
                ...
                Return SystemFunction.Mail.SendMail(ToAddress, CcAddress, "", "Formazione " & Formazione.Giornata & "° giornata " & Torneo & " - " & Globalization.CultureInfo.CurrentCulture.TextInfo.ToTitleCase(Formazione.Nome.ToLower), Body, att)
        ...
        Public Shared Function SendFormazioni(ToAddress As String, CcAddress As String, Formazioni As List(Of LegaObject.Formazione), Subject As String, Body As String) As Boolean

            If Formazioni.Count > 0 Then
                Dim fname As String = ImpExp.ImpExpFormazioni.ExportHtml(Formazioni(0).Giornata, Formazioni, "", False, True)
                ...
                If fname <> "" Then
                    ...
                    Return SystemFunction.Mail.SendMail(ToAddress, CcAddress, "", Subject, Body, att)
        ...
        Public Shared Function SendMail(ToAddress As String, CcAddress As String, Display As String, Subject As String, Body As String, AttachFileList As List(Of String)) As Boolean
            ...
            Dim cred As New Net.NetworkCredential("formazioni@ifantacalcio.it", "sanvito01#")
            ...
            Try
                ...
                Else
                    ...
                    Call WriteError(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType.FullName, System.Reflection.MethodBase.GetCurrentMethod().Name, "Mail administrator lega missing or not valid")
                    ris = False
