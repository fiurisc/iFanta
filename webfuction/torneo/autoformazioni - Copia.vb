    ...
    Public Class AutoFormazioniData
        ...
        Public Function GetFormazioneAutomatica(ByVal Giornata As Integer, ByVal IdTeam As Integer, hist As List(Of AutoFormazione.ParamenterValues)) As AutoFormazione
            ...
            Try
                ...
                SetDayData(Giornata)
                If Giornata > 1 AndAlso enapreanalisi Then
        ...
        Private Function PreAnalisi(ByVal Giornata As Integer, ByVal IdTeam As Integer, hist As List(Of AutoFormazione.ParamenterValues)) As AutoFormazione.ParamenterValues
            ...
            Dim avgmhistp As Integer = If(hist.Count > 0, CalcolaValoreMedio(hist.Select(Function(x) x.HistoricalPosistionData).ToList()), 20)
            ...
            For Each key1 As String In totresults.Keys
                For Each result As AutoFormazione In totresults(key1)
                    If result IsNot Nothing AndAlso result.Formazione.Punti = ptmaxprevfinal Then
                        Dim key As String = "re"
                        If ptmaxprev.ContainsKey(key) = False Then ptmaxprev.Add(key, 0)
            ...
            For Each key1 As String In ptmaxprev.Keys
                If ptmaxprev(key1) = ptmaxprevfinal Then
                    Dim key As String = "re"
                    Dim para As New AutoFormazione.ParamenterValues
        ...
        Public Function AnalisiReal(Giornata As Integer, IdTeam As Integer, results As Dictionary(Of String, List(Of AutoFormazione))) As List(Of AutoFormazione.ParamenterValues)
            ...
            For Each key As String In results.Keys
                For Each result As AutoFormazione In results(key)
                ...
                For Each result As AutoFormazione In results(key)
                    If result IsNot Nothing Then
                        If result.Formazione.Punti = ptmax Then
                            ...
                            'Debug.WriteLine("best:" & result.Parameters.BestPoints)
                            'For Each p As Torneo.FormazioniData.PlayerFormazione In result.Formazione.Players
                            '    If p.InCampo = 1 Then
                            '        Debug.WriteLine(p.Nome & vbTab & p.Punti)
                            '    End If
                            'Next
        ...
        Public Function CalcolaValoreMedio(valori As List(Of Integer), Optional LastValue As Integer = -1) As Integer
            ...
            'Dim tot As Integer = valori.Sum
            'Dim ntot As Integer = valori.Count

            If ntot > 0 Then
                Return CInt(tot / ntot)
            Else
                Return 0
            End If
