...
Namespace Torneo
    Public Class ClassificaData
        ...
        Public Function GetStoricoData(top As Boolean) As Dictionary(Of String, StoricoDati)
            ...
            str.AppendLine("SELECT f.gio,f.idteam,'0' as pt,'0' as ptmin,sum(f.pt) as ptmax,'0' as amm,'0' as esp,'0' as ass,'0' as gs,'0' as gf")
            str.AppendLine("FROM tbformazionitop as f")
            str.AppendLine("WHERE (incampo=1 OR type=10) and f.pt>-100")
            str.AppendLine("GROUP BY f.idteam,f.gio")
            str.AppendLine("UNION")
            str.AppendLine("SELECT f.gio,f.idteam,'0' as pt,sum(f.pt) as ptmin,'0' as ptmax,'0' as amm,'0' as esp,'0' as ass,'0' as gs,'0' as gf")
            str.AppendLine("FROM tbformazioniflop as f")
            str.AppendLine("WHERE (incampo=1 OR type=10) and f.pt>-100")
            str.AppendLine("GROUP BY f.idteam,f.gio")
            str.AppendLine(") AS t LEFT JOIN tbteam as tt  ON t.idteam=tt.idteam")
