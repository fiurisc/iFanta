...
Namespace WebData
    ...
    Public Class PlayersData
        ...
        Public Function GetNatDataFileName() As String
            Return appSett.WebDataPath & "data\nat-data.json"
        End Function
        ...
        Public Function GetPlayersData(ReturnData As Boolean) As String
            ...
            Dim fileJsonNat As String = GetNatDataFileName()
            Dim fileLog1 As String = dirData & Path.GetFileNameWithoutExtension(GetDataFileName) & "_1.log"
            Dim fileLog2 As String = dirData & Path.GetFileNameWithoutExtension(GetDataFileName) & "_2.log"
            ...
            Try
                ...
                Dim dicTeam As New Dictionary(Of String, String) From {
                    {"ATALANTA", "708"},
                    {"BOLOGNA", "8513"},
                    {"CAGLIARI", "585"},
                    {"COMO", "268"},
                ...
                Dim natids As New Dictionary(Of String, String)

                If IO.File.Exists(fileJsonNat) Then
                    natids = Functions.DeserializeJson(Of Dictionary(Of String, String))(IO.File.ReadAllText(fileJsonNat))
                End If

                For i As Integer = 0 To team.Count - 1
                    If dicTeam.ContainsKey(team(i)) Then
                        sqlink.Add(team(i), "https://api.sportmonks.com/v3/football/squads/teams/" & dicTeam(team(i)) & "?api_token=" & Functions.token1 & "&season_id=" & Functions.seasonid(appSett.Year) & "&include=player;position")
                    End If
                Next

                For Each sq As String In sqlink.Keys
                    Try
                        Dim json As String = Functions.GetPage(appSett, sqlink(sq))
                        ...
                        If json <> "" Then
                            ...
                            Dim obj As Object = serializer.DeserializeObject(json)
                            Dim formattedJson As String = Functions.SerializzaOggetto(obj, True)
                            ...
                            IO.File.WriteAllText(fileTemp, formattedJson, System.Text.Encoding.GetEncoding("utf-8"))

                            Dim lines() As String = IO.File.ReadAllLines(fileTemp)
                            ...
                            For i As Integer = 0 To lines.Length - 1
                                ...
                                Dim pname As String = Functions.GetJsonPropertyName(lines(i))
                                Dim pvalue As String = Functions.GetJsonPropertyValue(lines(i))

                                If pvalue.ToUpper().Contains("VEIGA") Then
                                    pvalue = pvalue
                                End If
                                If pname = "common_name" Then
                                    name1 = Functions.NormalizeText(pvalue.ToUpper())
                                ElseIf pname = "firstname" Then
                                    name2 = Functions.NormalizeText(pvalue.ToUpper())
                                ElseIf pname = "lastname" Then
                                    name3 = Functions.NormalizeText(pvalue.ToUpper())
                                ...
                                ElseIf pname = "date_of_birth" Then
                                    ...
                                    If pvalue <> "" Then
                                        Dim birthday As Date = CDate(pvalue)
                                ...
                                ElseIf pname = "nationality_id" Then

                                    Dim natid As String = pvalue

                                    If natid <> "null" Then
                                        If natid <> "null" AndAlso natids.ContainsKey(natid) = False Then
                                            Try
                                                Dim jsonNat As String = Functions.GetPage(appSett, "https://api.sportmonks.com/v3/core/countries/" & natid & "?api_token=" & Functions.token1)
                                                Dim obj1 As Object = serializer.DeserializeObject(jsonNat)
                                                Dim formattedJsonNat As String = Functions.SerializzaOggetto(obj1, True)
                                                Dim linesNat() As String = formattedJsonNat.Split({Environment.NewLine}, StringSplitOptions.None)
                                                For Each line As String In linesNat
                                                    Dim pname1 As String = Functions.GetJsonPropertyName(line)
                                                    Dim pvalue1 As String = Functions.GetJsonPropertyValue(line)
                                                    If pname1 = "iso3" Then
                                                        nat = pvalue1
                                                        natids.Add(natid, pvalue1)
                                                    End If
                                                Next
                                ...
                                ElseIf pname = "code" Then

                                    If pvalue = "goalkeeper" Then
                                        role = "P"
                                    ElseIf pvalue = "defender" Then
                                        role = "D"
                                    ElseIf pvalue = "midfielder" Then
                                        role = "C"
                                    ElseIf pvalue = "attacker" Then
                                        role = "A"
                                    Else
                                        role = ""
                                    End If
                                    ...
                                    If role <> "" Then
                                        ...
                                        If name1.ToUpper().Contains("VEIGA") Then
                                            pvalue = pvalue
                                        End If

                                        Dim playerm As WebData.Players.PlayerMatch = WebData.Players.Data.ResolveName(role, name4, sq, wpl, True, False)
                                        If playerm.Matched = False Then playerm = WebData.Players.Data.ResolveName(role, name1, sq, wpl, True, False)
                                        If playerm.Matched = False Then playerm = WebData.Players.Data.ResolveName(role, name3, sq, wpl, True, False)
                                        If playerm.Matched = False Then playerm = WebData.Players.Data.ResolveName("", name4, sq, wpl, True, False)
                                        If playerm.Matched = False Then playerm = WebData.Players.Data.ResolveName("", name1, sq, wpl, True, False)
                                        If playerm.Matched = False Then playerm = WebData.Players.Data.ResolveName("", name3, sq, wpl, True, False)
                ...
                strdata = Functions.SerializzaOggetto(natids, False)

                IO.File.WriteAllText(fileJsonNat, strdata)

                If Functions.makefileplayer Then IO.File.WriteAllText(fileLog1, Functions.GetDataPlayerMatchedData(appSett, wpl, True), System.Text.Encoding.UTF8)
                If Functions.makefileplayer Then IO.File.WriteAllText(fileLog2, strplayer.ToString(), System.Text.Encoding.UTF8)
        ...
        Public Function GetPlayersDataOld(ReturnData As Boolean) As String
