...
Partial Class WebData
    ...
    Function GetPlayersQuotePianetaFantacalcio(ServerPath As String, ReturnData As Boolean) As String
        ...
        Try
            ...
            If html <> "" Then
                    ...
                    If line(i).Contains("</tr>") Then
                        If start Then
                            ...
                            For k As Integer = 0 To s.Length - 1
                                If s(k).Contains("images/transparent.gif"" class=""SpriteSquadre ") Then squadra = CheckTeamName(System.Text.RegularExpressions.Regex.Match(s(k), "(?<=title="")[a-zA-Z]{1,}(?="")").Value.ToUpper())
                                If s(k).ToLower.Contains("giocatori-statistiche-personali.asp?nomegio=") Then
                                    ...
                                    nome = ResolveName("", nome, squadra, wpl, True).GetName()
                                End If
                            Next
                        End If
                        start = False
                ...
                If makefileplayer Then IO.File.WriteAllText(filed, GetDataPlayerMatchedData(wpl, True), System.Text.Encoding.UTF8)

            End If

            If ReturnData Then
                Return "</br><span style=color:red;font-size:bold;'>Players quotes (" & year & "):</span></br>" & strdata.ToString.Replace(System.Environment.NewLine, "</br>") & "</br><span style='color:red;font-size:bold;'>Details:</span></br>" & GetDataPlayerMatchedData(wpl, False).Replace(System.Environment.NewLine, "</br>")
            Else
    ...
    Function GetPlayersQuoteFantacalcio(ServerPath As String, ReturnData As Boolean) As String
        ...
        Try
            ...
            If html <> "" Then
                ...
                For i As Integer = 0 To line.Length - 1
                    If line(i).Contains("data-filter-keywords") Then
                        nome = NormalizeText(System.Text.RegularExpressions.Regex.Match(line(i), "(?<="").*(?="")").Value.ToUpper().Trim().Replace("&#X27;", "'"))
                    End If
            ...
            webplayers.Clear()
            Call LoadWebPlayers(filedpq)

            If ReturnData Then
                Return "</br><span style=color:red;font-size:bold;'>Players quotes (" & year & "):</span></br>" & strdatapq.ToString.Replace(System.Environment.NewLine, "</br>") & "</br>"
            Else
                Return ("</br><span style=color:red;font-size:bold;'>Players quotes (" & year & "):</span><span style=color:blue;font-size:bold;'>Compleated!!</span></br>")
            End If
    ...
    Function GetPlayersQuoteFantacalcio2(ServerPath As String, ReturnData As Boolean) As String
        ...
        Try
            ...
            Dim html As String = GetPage("https://www.fantacalcio.it/quotazioni-fantacalcio", "POST", "")
            Dim link As String = ""
