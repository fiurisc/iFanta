...
Public Class Zip
    ...
    Public Shared Sub UpZipFile(ByVal ZipFileName As String, ByVal Directory As String)
        Dim dirtmp As String = My.Application.Info.DirectoryPath & "\temp\unzip"
        If IO.Directory.Exists(dirtmp) = False Then IO.Directory.CreateDirectory(dirtmp)
        Dim f() As String = IO.Directory.GetFiles(dirtmp)
        For i As Integer = 0 To f.Length - 1
            IO.File.Delete(f(i))
        Next
        System.IO.Compression.ZipFile.ExtractToDirectory(ZipFileName, dirtmp)
        f = IO.Directory.GetFiles(dirtmp)
        For i As Integer = 0 To f.Length - 1
            IO.File.Copy(f(i), IO.Path.GetDirectoryName(ZipFileName) & "\" & IO.Path.GetFileName(f(i)), True)
        Next
    End Sub
